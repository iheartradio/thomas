...
private[thomas] object Bucketing {
  ...
  def consolidateRanges(list: List[GroupRange]): List[GroupRange] = {
    list
      .sortBy(_.start)
      .foldLeft(List.empty[GroupRange]) { (memo, range) =>
        memo match {
          case head :: tail if head.end == range.start =>
            GroupRange(head.start, range.end) :: tail
          case _ => range :: memo
        }
      }
      .reverse
  ...
  def newRanges(
    ...
    if (consistent) oldRanges
      ...
      groupsInSurplusOrder.foldLeft(removeOutdatedAssignments) { (assigned, group) =>
        ...
        def allocate(
            neededSize: GroupSize,
            available: List[GroupRange]
          ): List[GroupRange] = {
          def loop(
              rest: GroupSize,
        ...
        val newAssignment =
          assigned.get(group.name).fold(allocate(group.size, allAvailable)) {
            assignment =>
