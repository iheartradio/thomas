...
package model {
  ...
  /** Data used to create an A/B tests
    ...
    * @param feature
    *   feature name of the treatment. This is an identifier with which feature code
    *   can determine for each user which treatment they get.
    * @param author
    *   author name. Can be used for ownership
    * @param start
    *   scheduled start of the test
    * @param end
    *   scheduled end of the test, optional, if not given the test will last
    *   indefinitely
    * @param groups
    *   group definitions. group sizes don't have to add up to 1, but they cannot go
    *   beyond 1. If the sum group size is less than 1, it means that there is a
    *   portion (1 - the sum group size) of users won't be the in tests at all, you
    *   could make this group your control group.
    * @param requiredTags
    *   an array of string tags for eligibility control. Once set, only users having
    *   these tags (tags are passed in group assignment inquiry requests) are
    *   eligible for this test.
    * @param alternativeIdName
    *   by default Thomas uses the "userId" field passed in the group assignment
    *   inquiry request as the unique identification for segmenting users. In some
    *   A/B test cases, e.g. some features in the user registration process, a user
    *   Id might not be given yet. In such cases, you can choose to use any user meta
    *   field as the unique identification for users.
    * @param userMetaCriteria
    *   this is more advanced eligibility control. You can set a field and criterion
    *   pair to match user meta. Only users whose metadata field value matches the
    *   criterion are eligible for the experiment.
    * @param reshuffle
    *   by default Thomas will try to keep the user assignment consistent between
    *   different versions of experiments of a feature. Setting this field to true
    *   will redistribute users again among all groups.
    * @param segmentRanges
