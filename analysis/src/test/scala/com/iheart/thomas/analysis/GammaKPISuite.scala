...
class GammaKPISuite extends FunSuite with Matchers {
  ...
  def mock(abTestData: Map[GroupName, Measurements] = Map(),
                                                 historical: Measurements = Nil): AbtestMeasurable[F, GammaKPIDistribution] = new AbtestMeasurable[F, GammaKPIDistribution] {
  ...
  test("Measure one group against control generates result") {
    ...
    val n = 1000
    implicit val measurable = mock(Map("A" -> Random.shuffle(Gamma(0.55, 3).param.sample()).take(n),
      "B" -> Random.shuffle(Gamma(0.5, 3).param.sample()).take(n)
    ))
    AbtestKPI[F, GammaKPIDistribution]
  ...
  test("Diagnostic trace") {
    ...
    val result = GammaKPIDistribution(KPIName("test"),
      Normal(0.5, 0.1),
      Normal(3, 0.1)
  ...
  test("updated with new piror") {
    ...
    implicit val measurable = mock(historical = Random.shuffle(Gamma(0.55, 3).param.sample(5000)))
