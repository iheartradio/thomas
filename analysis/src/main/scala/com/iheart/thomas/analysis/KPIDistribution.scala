...
case class BetaKPIDistribution(name: KPIName, alphaPrior: Double, betaPrior: Double)
    extends KPIDistribution

object BetaKPIDistribution {
  def sample(b: BetaKPIDistribution, data: Conversions): Indicator = {
    val postAlpha = b.alphaPrior + data.converted
    val postBeta = b.betaPrior + data.total - data.converted
    Beta(postAlpha, postBeta).param
  }

  implicit def betaInstances[F[_]](implicit
                                   sampleSettings: SampleSettings,
                                   rng: RNG,
                                   B: Measurable[F, Conversions, BetaKPIDistribution],
                                   F: MonadError[F, Throwable])
    : AssessmentAlg[F, BetaKPIDistribution] with UpdatableKPI[F, BetaKPIDistribution] =
    new BayesianAssessmentAlg[F, BetaKPIDistribution, Conversions]
    with UpdatableKPI[F, BetaKPIDistribution] {

      protected def sampleIndicator(b: BetaKPIDistribution, data: Conversions) =
        sample(b, data)

      def updateFromData(kpi: BetaKPIDistribution,
                         start: OffsetDateTime,
                         end: OffsetDateTime): F[(BetaKPIDistribution, Double)] =
  ...
  implicit def basicAssessmentAlg[F[_]](
      ...
      sampleSettings: SampleSettings,
      rng: RNG,
      F: Sync[F]): BasicAssessmentAlg[F, BetaKPIDistribution, Conversions] =
...
case class GammaKPIDistribution(name: KPIName, shapePrior: Normal, scalePrior: Normal)
    extends KPIDistribution {
...
object GammaKPIDistribution {
  ...
  implicit def gammaKPIInstances[F[_]](
      ...
      F: MonadError[F, Throwable])
    : AssessmentAlg[F, GammaKPIDistribution] with UpdatableKPI[F, GammaKPIDistribution] =
    ...
    with UpdatableKPI[F, GammaKPIDistribution] {
      ...
      private def fitModel(
          ...
          data: List[Double]): RandomVariable[(Real, Real, Distribution[Double])] =
        for {
      ...
      def updateFromData(k: GammaKPIDistribution,
        ...
        K.measureHistory(k, start, end).map { data =>
          ...
          val updated = k.copy(shapePrior = Normal.fit(shapeSample),
                               scalePrior = Normal.fit(scaleSample))

          val ksTest = new KolmogorovSmirnovTest()
          val gd = new GammaDistribution(updated.shapePrior.location,
