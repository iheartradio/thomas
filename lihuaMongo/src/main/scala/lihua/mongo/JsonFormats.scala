...
object JsonFormats extends playJson.Formats {
  ...
  object StringBooleanFormat extends Format[Boolean] {
    ...
    override def reads(json: JsValue): JsResult[Boolean] =
      json.validate[String].map(_.toLowerCase == "true")
  ...
  implicit def javaEnumFormats[ET <: Enum[ET]: ClassTag]: Format[ET] =
    Format(javaEnumReads[ET], javaEnumWrites[ET])
  ...
  object StringParser {
    ...
    implicit val stringStringParser: StringParser[String] =
