...
object BayesianMABCommands {
  ...
  private val banditSpecOpts =
    (fnOpts,
     ...
     Opts.option[Instant]("start", "start time of the MAB"),
     Opts.option[String]("title", "author name", "u")).mapN(BanditSpec.apply)

  def conversionBMABCommand[F[_]](implicit F: ConcurrentEffect[F], ec: ExecutionContext) =
    Command("conversionBMAB", "manage conversion based Bayesian Multi Arm Bandits")(
      Opts.subcommands(
        ...
        Command("show", "show an existing conversion KPI based Bayesian MAB") {
          (fnOpts, conversionClientOpts[F]).mapN { (feature, clientR) =>
            clientR.use { client =>
              client
                ...
                .flatMap(s =>
                  F.delay {
                    println("=========== Bayesian State Start ============")
                    println(s)
                    println("=========== Bayesian State End =============")
                })
            }
          }
        },
        Command("reallocate", "show an existing conversion KPI based Bayesian MAB") {
