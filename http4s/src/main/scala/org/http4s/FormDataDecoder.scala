...
object FormDataDecoder {
  ...
  implicit def formEntityDecoder[F[_]: Sync, A](
      implicit
    ...
    UrlForm.entityDecoder[F].flatMapR { d =>
      fdd(d.values)
        .leftMap(
          es => InvalidMessageBodyFailure(es.map(_.sanitized).mkString_("\n"))
        )
  ...
  implicit class FormDataDecoderSyntax[A](
      private val decoder: FormDataDecoder[Either[String, A]])
      extends AnyVal {
  ...
  def chainEither[A](
      key: String
    )(implicit A: FormDataDecoder[A]
    ): FormDataDecoder[Either[String, Chain[A]]] =
